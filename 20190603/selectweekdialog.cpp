...
SelectWeekDialog::SelectWeekDialog( QList< bool >       boolList,
                                    ...
                                    QWidget *           parent )
    : QDialog( parent ), delegate( invoker ),
      termTotalWeek( delegate->termTotalWeek ), ui( new Ui::selectWeekDialog ) {
    ui->setupUi( this );

    for ( int i = 1; i <= termTotalWeek; ++i ) {
